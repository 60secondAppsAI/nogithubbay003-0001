        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAdvertisements = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAdvertisements">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Advertisement</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AdvertisementId" type="text" placeholder="Enter AdvertisementId" v-model="advertisementToAdd.advertisementId"></base-input>
  <base-input label="StartDate" type="text" placeholder="Enter StartDate" v-model="advertisementToAdd.startDate"></base-input>
  <base-input label="EndDate" type="text" placeholder="Enter EndDate" v-model="advertisementToAdd.endDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="advertisements" :row-key="record => record.AdvertisementId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AdvertisementPictureView :advertisements="advertisements" />
          </div>
...
import AdvertisementPictureView from './AdvertisementPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    AdvertisementPictureView
  },

  data() {
    return {
      modalAdvertisements: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'advertisementId',
          visible: true,
          scopedSlots: { customRender: 'advertisementId' },
          sorter: true
          //sorter: (a, b) => a.advertisementId - b.advertisementId,
          //sorter: (a, b) => a.advertisementId.localeCompare(b.advertisementId),
        },
        {
          title: 'Start Date',
		dataIndex: 'startDate',
      ...
      sortBy: 'advertisementId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAdvertisementsTable() {
      ...
      let advertisementsTableData = [];
      for (let i = 0; i < this.advertisements.length; i++) {
        advertisementsTableData.push({
          id: i,
          advertisementId: this.advertisements[i].advertisementId,
          year: this.advertisements[i].year,
          date: this.advertisements[i].date,
          competitionId: this.advertisements[i].competitionId,
          advertisementId: this.advertisements[i].advertisementId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-advertisements',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAdvertisementsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAdvertisements = false;

      const currentDate = new Date().getTime();
      this.advertisementToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.advertisementToAdd);
      console.log(jsonData);
      
      const res = await AdvertisementService.addAdvertisement(jsonData);
